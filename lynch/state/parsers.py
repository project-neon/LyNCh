...
from lynch.state.schema import Transition, FrameState, BallState, RobotState
...
class JSONParser:
    @staticmethod
    def _parse_state_vector(vector: List[float]) -> Optional[FrameState]:
        """Convert a 16-float NeonFC state vector into a FrameState."""
        ...
        return FrameState(ball=ball, robots=robots)

    @classmethod
    def parse_from_bytes(cls, data) -> Optional[Transition]:
        try:
            ...
            if not isinstance(cur, list):
                logger.error(f"NeonFC cur_state is not a list, got {type(cur)}: {cur}")
                return None
...
class ProtobufParser:
    ...
    @staticmethod
    def _extract_state(frame) -> FrameState:
        """Normalize SSL Vision TrackedFrame into the canonical FrameState."""
        ball = None
        if frame.balls:
            b = frame.balls[0]
            ball = BallState(
                x=b.pos.x,
                y=b.pos.y,
                vx=b.vel.x if b.HasField("vel") else 0.0,
                vy=b.vel.y if b.HasField("vel") else 0.0,
            )
